...
void processScaffold(vector<string> &FASTA_headers, vector<string> &FASTA_sequences, map<unsigned long, unsigned long> &population_map, unsigned long num_pops, map<unsigned long, unsigned long> &inbred, unsigned int debug, bool usable) {
   ...
   for (unsigned long i = 0; i < scaffold_length; i++) {
      ...
      for (unsigned long pop_index = 0; pop_index < num_pops; pop_index++) {
         ...
         //Verbose debugging:
         if (debug > 1) {
            cerr << FASTA_headers[0].substr(1) << '\t' << i+1 << '\t' << pop_index << endl;
         }
   ...
   while (readFASTAs(input_FASTAs, FASTA_lines)) {
      ...
      if (all_header_lines) { //Process the previous scaffold if at a header
         if (!FASTA_sequences.empty()) {
            processScaffold(FASTA_headers, FASTA_sequences, population_map, num_pops, inbred, debug, usable);
