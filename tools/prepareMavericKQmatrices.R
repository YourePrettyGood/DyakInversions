...
BEDfile <- options[3];
#Fixed width of the suffixes for window IDs, typically 5:
sufwidth <- options[4];
...
cat(paste0("Using 0-based window suffix of fixed width: ", sufwidth, "\n"))
...
for (h in seq(1, nrow(window_bed))) {
   ...
   window_suffix <- formatC(h-1, width=sufwidth, flag="0")
   #Read in the log-likelihoods for each K:
   Kchoice <- read.csv(paste0("Evidence/", prefix, "_window", window_suffix, "_Evidence.csv"));
   ...
   if (length(K) > 1) {
      cat(paste0("Found multiple best K for window ", window_suffix))
      #Choose the first K, arbitrarily, since it should be the smallest K
      K <- K[1]
   }
   #Read in the Q matrix for the best K:
   Qmatrix <- read.csv(paste0("QMatrices/", prefix, "_strictest_window", window_suffix, "_Qmatrix_perInd_K", K, ".csv"));
